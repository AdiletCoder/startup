...
class FavoriteToVisitSerializer(serializers.ModelSerializer):
    class Meta:
        model = FavoriteToVisit
        fields = ('id', 'visit', 'user', 'favorite')
    ...
    def create(self, validated_data):
        ...
        user = request.user
        visit = validated_data.get('visit')
        favorite = FavoriteToVisit.objects.get_or_create(user=user, visit=visit)[0]
...
class PlaceToEatSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        representation['comments'] = CommentToEatSerializer(instance.comments_to_eat.all(), many=True, context=self.context).data
...
class FavoriteToEatSerializer(serializers.ModelSerializer):
    class Meta:
        model = FavoriteToEat
        fields = ('id', 'eat', 'user', 'favorite')
    ...
    def create(self, validated_data):
        ...
        user = request.user
        eat = validated_data.get('eat')
        favorite = FavoriteToEat.objects.get_or_create(user=user, eat=eat)[0]
        favorite.favorite = True if favorite.favorite == False else False
        favorite.save()
        return favorite
...
class PlaceToStaySerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        representation = super().to_representation(instance)
        representation['images'] = PlaceToStayImageSerializer(instance.images_to_stay.all(), many=True, context=self.context).data
        representation['comments'] = CommentToStaySerializer(instance.comments_to_stay.all(), many=True, context=self.context).data
        representation['likes'] = len(LikeToStaySerializer(instance.likes_to_stay.filter(like=True), many=True, context=self.context).data)
        representation['rating'] = RatingToStaySerializer(instance.ratings_to_stay.all(), many=True, context=self.context).data
        return representation
...
class FavoriteToStaySerializer(serializers.ModelSerializer):
    class Meta:
        model = FavoriteToStay
        fields = ('id', 'stay', 'user', 'favorite')
    ...
    def create(self, validated_data):
        ...
        favorite = FavoriteToStay.objects.get_or_create(user=user, stay=stay)[0]
        favorite.favorite = True if favorite.favorite == False else False
        favorite.save()
        return favorite
